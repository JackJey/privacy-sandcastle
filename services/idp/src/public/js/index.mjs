...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const { verifiable, conditional } = await passkeySupport()
  $("#verifiable").textContent = verifiable ? "supported" : "not supported"
  $("#conditional").textContent = conditional ? "supported" : "not supported"

  $("#registerCredential").addEventListener("submit", async (e) => {
    e.preventDefault()
    const publicKey = await (await fetch("/credential/publickey")).json()
    ...
    const body = {
      ...
      response: {
        ...
        attestationObject: base64url.encode(credentials.response.attestationObject),
